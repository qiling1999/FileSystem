...
// OnFileUploadFinished : 文件上传完成，保存meta到MySQL表里面去
func OnFileUploadFinished(filehash string, filename string,
	filesize int64, fileaddr string) bool {
	stmt, err := mydb.DBConn().Prepare(  //写sql语句
	...
	ret, err := stmt.Exec(filehash, filename, filesize, fileaddr)//真正执行sql语句
	if err != nil {
...
func GetFileMeta(filehash string) (*TableFile, error) {//返回值为file哈希对应的一条文件元信息的记录
